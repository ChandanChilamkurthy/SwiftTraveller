...
public final class Traveller {
    ...
    private lazy var rootController: (TravellerStoryBoardProtocol, ControllerDestination) -> UIViewController? = { (story, controller) in
        ...
        if  let rootViewContoller = story.instantiateInitialViewController()  {
            ...
            if let viewController = UIViewController.makeViewController(for: controller, storyBoard: story, modelPresentationStyle: UIModalPresentationStyle.none, modelTransistionStyle: .none) {
...
extension Traveller {
    ...
    public enum route: Equatable {
         ...
         */
        case push(controller: ControllerDestination, animated: Bool, hidesTopBar: Bool, hidesBottomBar: Bool, modelTransistion: UIModalTransitionStyle, modelPresentation: UIModalPresentationStyle)
        ...
        /**
         ...
         !* @note:  In-Order to use this we need to wire segues in StoryBoard.
         */
        case unwind(destination: ControllerDestination, modelTransistionStyle: UIModalTransitionStyle, storyBoardSegue: TravellerStoryBoardSegueProtocol)
        ...
        public func perform<T>(_ configure: ((T) -> Void)? = nil) -> T? where T: UIViewController {
            switch self {
                ...
                case let .unwind(destination, modelTransistionStyle, storyBoardSegue):
                    return Traveller.shared.wayFinding?.unwind(to: destination, modelTransistionStyle: modelTransistionStyle, storyBoardSegue: storyBoardSegue, configure: configure)
            }
            return nil
        }
